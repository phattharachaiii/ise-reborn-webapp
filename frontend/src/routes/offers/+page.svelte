...
<script lang="ts">
	...
	function tabClass(tab: RoleTab) {
		const isActive = role === tab;
		return [
			// base
			'px-3 py-1.5 rounded-md text-sm border transition-colors select-none',
			'focus:outline-none focus-visible:ring-2',

			// แยกตามสถานะ
			isActive
				? // แอคทีฟ = พื้นส้ม, hover เข้มขึ้นจาก .bg-brand:hover (มีอยู่แล้วใน CSS)
					'bg-brand text-white border-surface focus-visible:ring-[var(--color-brand-orange)] cursor-default pointer-events-none'
				: // ไม่แอคทีฟ = พื้นขาว/เทาอ่อน + hover แสดงชัดขึ้น
					[
						'bg-surface-white text-text-base border-surface cursor-pointer',
						'hover:bg-surface-light',
						'hover:text-[var(--color-brand-orange)]',
		...
		<div class="flex flex-wrap items-center gap-2">
			...
			<div class="inline-flex rounded-lg border border-surface p-1 bg-surface-white shadow-card/0">
				<button
					...
					on:click={() => setRole('all')}
				>
					All
				</button>
				<button
					class={tabClass('buyer')}
					aria-pressed={role === 'buyer'}
					on:click={() => setRole('buyer')}
				>
					Buyer
				</button>
				<button
					class={tabClass('seller')}
					aria-pressed={role === 'seller'}
					on:click={() => setRole('seller')}
				>
					Seller
				</button>
			</div>
			...
			<!-- status select -->
			<select
				class="cursor-pointer rounded border border-surface px-3 py-1.5 text-sm bg-surface-white focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-300"
				bind:value={status}
